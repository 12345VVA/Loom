...
- **Windows 终端防挂起习惯**：在 Windows 系统上，AI 启动终端命令行运行非交互式任务时，经常会由于进程不退出或等待人工交互而卡在 `Running` 状态。请遵循以下处理策略：
  1. **优先使用 `cmd /c`**：对于单次执行且不需要保持交互的命令，包裹在 `cmd /c "你的命令"` 中，确保执行完毕后自动释放。
  2. **发送 `exit` 指令**：如果已经通过 `manage_task` 启动了 PowerShell 会话且卡住，使用 `send_input` 操作发送 `exit\n` 来正常退出会话。
  3. **强制终止**：如果遇到死循环或不需要前台等待的常驻服务阻塞，使用 `manage_task` 工具的 `kill` 强行终止。
  4. **避免等待人工输入**：运行安装或测试命令时，务必携带强制确认标志（如 `-y`, `--yes`, `--force`），避免终端在后台静默等待 `(Y/N)` 导致挂起。
