...
- **Windows 终端防挂起习惯**：在 Windows 系统上，AI 启动终端命令行运行非交互式任务时，经常会因为交互式 PowerShell 进程不退出而卡在 `Running` 状态。AI 在执行此类命令后应在适当时机发送 `exit` 输入以关闭会话，或在可能的情况下优先采用 `cmd /c` 执行命令，以确保进程能够正常发送终止信号并释放。
