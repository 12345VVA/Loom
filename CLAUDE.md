...
**业务模块** (`app/modules/`): `base`（权限/认证）、`ai`（AI 模型）、`demo`、`dict`（字典）、`media`（媒体资源）、`notification`（通知）、`task`（异步任务）。每个模块包含 `config.py`、`controller/`、`service/`、`model/`、可选 `menu.json`。
...
**业务模块** (`src/modules/`): `ai`、`base`、`dict`、`media`、`notification`、`task`。每个模块含 `config.ts`、`index.ts`、`views/`。
