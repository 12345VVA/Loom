...
function debugRoute(label: string, payload?: any) {
	console.groupCollapsed(`[router] ${label}`);
		...
		if (!route.component) {
			...
			if (viewPath) {
				if (viewPath.startsWith('http')) {
					...
					debugRoute('append.resolveComponent', {
						path: route.path,
						name: route.name,
						viewPath,
						resolved: Boolean(route.component)
					});
				}
...
router.find = function (path: string) {
	...
	// 优先精确匹配，避免被首页、重定向或兜底路由误命中
	const exactRoute = routeList.find(route => {
		return formatPath(route.path) === targetPath && route.name !== 'index';
	});

	if (exactRoute) {
		debugRoute('find.exact', {
			path,
			targetPath,
			name: exactRoute.name,
			routePath: exactRoute.path,
			viewPath: exactRoute.viewPath,
			isReg: !!exactRoute.isReg
		});
...
router.beforeEach(async (to, from, next) => {
	...
	// 如果路由不存在
	if (!route) {
		debugRoute('beforeEach.noRoute', {
			to: to.fullPath,
			redirect: user.token ? '/404' : '/login'
		});
	...
	// 如果路由未注册
	if (!isReg) {
		debugRoute('beforeEach.appendRoute', {
			to: to.fullPath,
			route: {
				name: route.name,
				path: route.path,
	...
	if (user.token) {
		if (to.path === '/login') {
			...
			if (!storage.isExpired('token')) {
				debugRoute('beforeEach.redirectHomeFromLogin', {
					to: to.fullPath
				});
				next('/');
				return;
		...
		if (!config.ignore.token.some(ignorePath => to.path === ignorePath)) {
			debugRoute('beforeEach.redirectLogin', {
				to: to.fullPath
			});
			next('/login');
			return;
