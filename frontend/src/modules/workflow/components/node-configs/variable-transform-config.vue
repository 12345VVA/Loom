<template>
	<div class="variable-transform-config">
		<node-config-section :title="$t('转换配置')">
			...
			<el-form-item
				v-if="config.transform_type === 'extract_json_path'"
				:label="$t('JSON 路径 (Path)')"
				required
			>
				<cl-variable-input
					v-model="config.transform_args.path"
			...
			<el-form-item
				v-if="config.transform_type === 'eval_expression'"
				:label="$t('Python 表达式')"
				required
			>
				<cl-variable-input
					v-model="config.transform_args.expression"
					type="textarea"
					:rows="3"
