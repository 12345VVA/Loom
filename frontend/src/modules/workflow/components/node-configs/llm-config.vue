<template>
	...
	<node-config-section :title="$t('输出')" :tooltip="$t('定义模型输出的内容及变量名')">
		<template #actions>
			<el-dropdown trigger="click" popper-class="wf-output-format-dropdown" @command="config.outputFormat = $event">
				<span class="output-format-btn">
						...
						<template #content>
							<div style="max-width: 200px; line-height: 1.4;">
								<template v-if="config.outputFormat === 'json'">
									Schema 约束模式：系统自动生成 JSON Schema 约束模型输出，下游可用 {变量名.字段} 访问子字段。
								</template>
								<template v-else-if="config.outputFormat === 'json_object'">
									宽松模式：要求模型输出合法 JSON，但不限定 Schema。需在 Prompt 中描述所需 JSON 结构。
								</template>
								<template v-else>
									普通文本输出。
								</template>
					...
					<span style="margin: 0 4px;">{{ $t('输出格式') }} {{ config.outputFormat === 'json' ? 'JSON' : (config.outputFormat === 'json_object' ? 'JSON(宽松)' : '文本') }}</span>
					<el-icon><ArrowDown /></el-icon>
				</span>
				<template #dropdown>
					<el-dropdown-menu>
						<el-dropdown-item command="text" :class="{ 'is-active': config.outputFormat === 'text' }">{{ $t('文本') }}</el-dropdown-item>
						<el-dropdown-item command="json" :class="{ 'is-active': config.outputFormat === 'json' }">{{ $t('JSON') }}</el-dropdown-item>
						<el-dropdown-item command="json_object" :class="{ 'is-active': config.outputFormat === 'json_object' }">{{ $t('JSON (宽松)') }}</el-dropdown-item>
					</el-dropdown-menu>
				</template>
			</el-dropdown>
		...
		<template v-if="config.outputFormat === 'json'">
			<cl-json-tree-editor ref="jsonEditorRef" v-model="config.jsonFields" mode="schema" :hide-footer="true" />
		</template>

		<el-form-item :label="$t('输出变量写入')" required :style="{ marginTop: config.outputFormat === 'json' ? '16px' : '0' }">
