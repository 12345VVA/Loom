<template>
	<el-form-item :label="$t('生图大模型配置 (Profile)')" required>
	...
	<el-form-item :label="$t('参考图片 (图生图)')">
		...
		/>
		<div class="field-hint">仅火山方舟、阿里百炼、OpenAI 兼容渠道等支持图生图的模型下生效。</div>
	</el-form-item>

	<el-form-item :label="$t('图片尺寸 (size)')">
	...
	<el-collapse>
		<el-collapse-item :title="$t('高级参数 JSON')" name="advanced">
			...
			<el-input v-model="config.optionsJson" type="textarea" :rows="5" placeholder='例如: {"quality": "hd", "n": 2}' />
		</el-collapse-item>
	</el-collapse>

	<el-form-item :label="$t('图片 URL 写入变量')" required>
...
const providerKind = computed(() => {
	...
	if (adapter === 'bailian' || providerCode === 'bailian' || modelCode.includes('wan2.') || modelCode.includes('wanx')) return 'bailian';
	if (adapter === 'volcengine-ark' || providerCode.includes('volcengine') || modelCode.includes('seedream') || modelCode.includes('doubao')) return 'volcengine-ark';
	if (adapter === 'openai-compatible' || providerCode.includes('openai')) return 'openai';
	if (adapter === 'qianfan' || providerCode.includes('qianfan') || modelCode.includes('ernie') || modelCode.includes('irag')) return 'qianfan';
	if (adapter === 'gemini' || providerCode.includes('gemini') || modelCode.includes('gemini')) return 'gemini';
	return 'unknown';
});
...
const availableSizeOptions = computed(() => {
	...
	if (providerKind.value === 'volcengine-ark') {
		...
		if (code.includes('seedream-4-5') || code.includes('seedream-4-0')) return volcengineSeedream4SizeOptions;
