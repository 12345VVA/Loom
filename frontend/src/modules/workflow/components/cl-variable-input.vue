				...
				<el-popover placement="bottom-end" :width="280" trigger="click">
					...
					<div class="variable-list">
						<div v-if="loopContextVars?.length" class="var-group">
							...
							<div v-for="v in loopContextVars" :key="v.key" class="var-item" @click="insert(v.refText)">
								<span>{{ v.display }}</span>
						...
						<div v-if="upstreamOutputVars?.length" class="var-group">
							...
							<div v-for="v in upstreamOutputVars" :key="v.key" class="var-item" @click="insert(v.refText)">
								<span>{{ v.display }}</span>
								<small>{{ v.nodeLabel }}</small>
							</div>
						</div>
						<div v-if="!loopContextVars?.length && !upstreamOutputVars?.length" class="empty-hint">
							{{ $t('暂无可用变量') }}
						</div>
					</div>
				</el-popover>
...
function insert(refText: string) {
	...
	setTimeout(() => {
		const newEl = inputRef.value?.$el?.querySelector('input, textarea') as HTMLInputElement | HTMLTextAreaElement;
		if (newEl) {
			newEl.focus();
			newEl.setSelectionRange(start + refText.length, start + refText.length);
		}
	}, 0);
}
</script>
