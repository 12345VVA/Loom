...
**EPS 全名可理解为 Entity Protocol Specification**，它是 `cool-admin` 体系引以为傲的最**核心技术**之一。
...
### 1. 后端：反射机制收集 (数据抽取原理)
在 `cool-admin-midway` 这类强类型的后端架构中，借助于 TypeScript 和 Midway.js 框架自带的 **装饰器 (Decorators)** 以及 **原生 ES 标准 Reflect Metadata (元数据反射)**。
...
### 2. 前端：自动构建机理 (反向生成机理)
`cool-admin-vue` 前端体系会在执行启动（比如 `npm run dev` 运行或构建时）发起预请求。
...
EPS 服务给出的标准 JSON 数据大致结构必须符合以下规约（以 cool-admin 标准为例，仅做结构展示）：
