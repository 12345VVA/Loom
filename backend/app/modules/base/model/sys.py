...
class SysLog(BaseEntity, table=True):
    ...
    params: str | None = None  # 请求参数 (JSON)
    ip: str | None = None
    ip_addr: str | None = None
    status: int = Field(default=1)  # 0: 失败, 1: 成功
    message: str | None = None  # 结果细节
...
class SysSecurityLog(BaseEntity, table=True):
    ...
    operator_ip: str | None = None  # 操作者IP地址

    # 操作对象信息
    target_type: str = Field(index=True)  # 操作对象类型: user/role/menu/department
    target_id: int | None = Field(default=None, index=True)  # 操作对象ID
    target_name: str | None = None  # 操作对象名称（用于快速查询）
    ...
    resource_path: str | None = None  # 资源路径

    # 变更数据（JSON格式）
    old_value: str | None = None  # 变更前的数据（脱敏后）
    new_value: str | None = None  # 变更后的数据（脱敏后）
    diff_data: str | None = None  # 变更差异（JSON格式）

    # 审计信息
    business_type: str | None = None  # 业务类型（用于分类查询）
    request_id: str | None = Field(default=None, index=True)  # 关联请求ID
